const PROVINCE_CITY_GROUPS = {
  ...
  '湖北省': ['武汉', '黄石', '十堰', '宜昌', '襄阳', '鄂州', '荆门', '荆州', '黄冈', '孝感', '咸宁', '随州', '恩施', '仙桃', '潜江', '天门', '神农架'],
  ...
  '云南省': ['昆明', '曲靖', '玉溪', '保山', '昭通', '丽江', '普洱', '临沧', '楚雄', '红河', '文山', '西双版纳', '大理', '大理州', '德宏', '怒江', '迪庆'],
  '陕西省': ['西安', '铜川', '宝鸡', '咸阳', '渭南', '延安', '汉中', '榆林', '安康', '商洛', '杨凌'],
...
export const CITY_TO_PROVINCE_MAP = Object.entries(PROVINCE_CITY_GROUPS).reduce((acc, [province, cities]) => {
  cities.forEach((city) => {
    acc[city] = province
  })
  return acc
...
export function getProvinceByCity(city) {
  ...
  return CITY_TO_PROVINCE_MAP[normalized] || ''
}

export function getProvinceGroups() {
  return PROVINCE_CITY_GROUPS
}
